...
//         long do you envision a pixel is in your scene). - want to stick to (0, 800) since it looks the best in my opinion. - DONE
...
void update(float dt){
  ...
  if (leftPressed) obstacleVel = (new Vec2(-obstacleSpeed,0)).plus(obstacleVel);
  if (rightPressed) obstacleVel = (new Vec2(obstacleSpeed,0)).plus(obstacleVel);
  if (upPressed) obstacleVel = (new Vec2(0,-obstacleSpeed)).plus(obstacleVel);
  if (downPressed) obstacleVel = (new Vec2(0,obstacleSpeed)).plus(obstacleVel);
  if (obstacleVel.x != 0 && obstacleVel.y != 0) {
    obstacleVel.mul(abs((obstacleVel.normalized()).x));
  }
  if (shiftPressed) obstacleVel = obstacleVel.plus(obstacleVel);
  ...
  for (int i = 0; i <  numParticles; i++){
    ...
    if (pos[i].distanceTo(spherePos) < (sphereRadius+r)){
      ...
      vel[i].add(obstacleVel.times(1.2));
    }
  }
