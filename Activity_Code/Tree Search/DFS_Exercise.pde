...
/*
 TODO: 
    1. Try to understand how this Breadth-first Search (BFS) implementation works.
       ...
       How do I represent nodes? How do I represent edges? - nodes are represented as an array, with the edges being represented as their neighbors being in a specific space in the array of arrays
       What is the purpose of the visited list? What about the parent list? - visited list is to check to make sure not checked more than once. parent list to check which is the most optimal
       What is getting added to the fringe? In what order? - nodes that are being checked are added to the fringe
       How do I find the path once I've found the goal? - go down the parent list
    2. Convert this Breadth-first Search to a Depth-First Search.
       Which version BFS or DFS has a smaller maximum fring size? same?
...
while (fringe.size() > 0){
  int fringeTop = 0;
  int currentNode = fringe.get(fringeTop);
  if (neighbors[currentNode].size() == 0) {
    fringe.remove(fringeTop);
  }
  if (currentNode == goal){
    println("Goal found!");
  ...
  for (int i = 0; i < neighbors[currentNode].size(); i++){
    ...
    if (!visited[neighborNode]){
      visited[neighborNode] = true;
      parent[neighborNode] = currentNode;
      fringe.add(0, neighborNode);
      println("Added node", neighborNode, "to the fringe.");
      println(" Current Fringe: ", fringe);
      i = neighbors[currentNode].size();
    }
  } 
}
