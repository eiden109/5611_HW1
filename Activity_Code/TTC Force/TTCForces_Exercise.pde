...
TODO:
  ...
  1a. Scale the goal force by the value k_goal. Find a value of k_goal that lets the agents
     ...
     i.e., goal_force = k_goal*(goal_vel-current_vel); acc += goal_force - DONE (k_goal 3.25 seems to make it stop at the goal)
  2. Finish the function computeTTC() so that it computes the time-to-collision (TTC) between
     ...
     If there is no collision, return a TTC of -1. - DONE
     ie., TTC(A,B) = Ray-Disk(A's position, relative velocity, B's position, combined radius)
  2a. Test computeTTC() with some scenarios where you know the (approximate) true value. - DONE
  3. Compute an avoidance force as follows:
      ...
      very large value of k_goal to avoid collisions between agents. - DONE
   4. Make the following simulation improvements:
     - Draw agents with a slightly smaller radius than the one used for collisions - DONE
     - When an agent reaches its goal, stop it from reacting to other agents - DONE
   5. Finally, place 2 more agents in the scene and try to make an interesting scenario where the
      agents interact with each other. - DONE
...
float k_goal = 3.25;  //TODO: Tune this parameter to agent stop naturally on their goals
float k_avoid = 85;
float agentRad = 30; //default was 40
float goalSpeed = 100;
...
void setup(){
  ...
  agentPos[3] = new Vec2(600,600);
  agentPos[4] = new Vec2(200,450);
  goalPos[0] = new Vec2(200,420);
  goalPos[1] = new Vec2(120,120);
  goalPos[2] = new Vec2(220,220);
  goalPos[3] = new Vec2(500,350);
  goalPos[4] = new Vec2(600,500);
...
float computeTTC(Vec2 pos1, Vec2 vel1, float radius1, Vec2 pos2, Vec2 vel2, float radius2){
  Vec2 helper = vel1.plus(vel2);
  return rayCircleIntersectTime(pos1, (radius1 + radius2), pos2, helper);
}
...
void moveAgent(float dt){
  ...
  for (int i = 0; i < numAgents; i++){
    ...
    if ((agentPos[i].minus(goalPos[i])).x > 1 && (agentPos[i].minus(goalPos[i])).y > 1) {
      for (int j = 0; j < numAgents; j++) {
        ...
        float ttc = computeTTC(agentPos[i], agentVel[i], agentRad, agentPos[j], agentVel[j], agentRad);
          if (ttc != -1) {
            ...
            Vec2 norm = (A_future.minus(B_future)).normalized();
            agentAcc[i].add(norm.times(k_avoid * (1/ttc)));
          }
        }
      }
    }
